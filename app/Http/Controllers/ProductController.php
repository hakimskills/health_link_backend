...
class ProductController extends Controller
{
    ...
    {
        \Log::info('Starting used equipment creation', ['request_data' => $request->all()]);
        ...
        \DB::beginTransaction();
        try {
            \Log::info('Creating used equipment product');
            $product = \App\Models\Product::create($validated);
            \Log::info('Used equipment product created', ['product_id' => $product->product_id, 'attributes' => $product->toArray()]);
            ...
            if ($request->hasFile('images') && is_array($request->file('images'))) {
                \Log::info('Processing used equipment images', ['image_count' => count($request->file('images'))]);
                foreach ($request->file('images') as $index => $image) {
                    if ($image->isValid()) {
                        ...
                        \Log::info('Used equipment image stored', ['path' => $path, 'filename' => $image->getClientOriginalName()]);
                        ...
                        \Log::info('Used equipment ProductImage created', ['image_id' => $productImage->id]);
            ...
            \DB::commit();
            \Log::info('Used equipment database transaction committed', ['product_id' => $product->product_id]);
    
            // Ensure ProductImage records are visible to Flask
            \DB::connection()->getPdo()->exec('FLUSH TABLES product_images');
    
            // Call Flask to extract features for used equipment
            if (!empty($multipartData)) {
                ...
                \Log::info('Sending used equipment images to Flask for feature extraction', ['image_count' => count($imageIds)]);
                ...
                while ($retryCount < $maxRetries && !$success) {
                    try {
                        \Log::info('Attempting Flask request for used equipment', ['attempt' => $retryCount + 1]);
                        ...
                        \Log::info('Flask feature extraction successful for used equipment', [
                        ...
                        if (!isset($featureData['features']) || !is_array($featureData['features'])) {
                            \Log::warning('No features found in Flask response for used equipment', ['response' => $featureData]);
                        }
                    } catch (\GuzzleHttp\Exception\RequestException $e) {
                        \Log::warning('Flask request failed for used equipment', [
                            'error' => $e->getMessage(),
                ...
                if (!$success) {
                    \Log::error('Flask feature extraction failed after retries for used equipment', ['image_ids' => $imageIds]);
                }
            }
    
            \Log::info('Used equipment creation completed', ['product_id' => $product->product_id, 'image_count' => count($imageIds)]);
            return response()->json([
                'message' => 'Used equipment created successfully',
                'data' => $product->load('images')
            ], 201);
        } catch (\Exception $e) {
            \DB::rollBack();
            \Log::error('Used equipment creation failed: ' . $e->getMessage(), ['exception' => $e->getTraceAsString()]);
            return response()->json(['error' => 'Used equipment creation failed: ' . $e->getMessage()], 500);
        }
    }
